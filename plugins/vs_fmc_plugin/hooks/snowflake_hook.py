...
class SnowflakeHook(DbApiHook):
    ...
    def _get_conn_params(self) -> Dict[str, Optional[str]]:
        ...
        # Ensure session_parameters is a dict or None (not a string)
        if isinstance(session_parameters, str):
            import json
            try:
                session_parameters = json.loads(session_parameters) if session_parameters else None
            except json.JSONDecodeError:
                session_parameters = None
        
        final_session_params = self.session_parameters or session_parameters
        if not isinstance(final_session_params, dict):
            final_session_params = None
        ...
        conn_config = {
            ...
            "session_parameters": final_session_params,
        ...
        # Support multiple ways to specify private key:
        # 1. private_key_file - original VaultSpeed way (file path)
        # 2. extra__snowflake__private_key_path - Astronomer UI "Private Key Path" field
        # 3. private_key_content / extra__snowflake__private_key_content - key content directly
        private_key_file = (
            conn.extra_dejson.get('private_key_file') or 
            conn.extra_dejson.get('extra__snowflake__private_key_path') or
            conn.extra_dejson.get('private_key_path')
        )
        private_key_content = (
            conn.extra_dejson.get('private_key_content') or 
            conn.extra_dejson.get('extra__snowflake__private_key_content')
        )
        ...
        if private_key_file:
            ...
            # Load from content directly (Astronomer UI)
            key_bytes = private_key_content.encode() if isinstance(private_key_content, str) else private_key_content
            p_key = serialization.load_pem_private_key(
