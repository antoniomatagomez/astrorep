...
Vaultspeed version: 5.7.4.21, generation date: 2025/12/18 09:15:50
...
default_args = {
	...
	"start_date":datetime.strptime("05-09-2024 12:00:00", "%d-%m-%Y %H:%M:%S")
}

path_to_mtd = Path(Variable.get("path_to_metadata"))

if (path_to_mtd / "768_mappings_MMOSA_MKTG_INiT_20251218_091550.json").exists():
	with open(path_to_mtd / "768_mappings_MMOSA_MKTG_INiT_20251218_091550.json") as file: 
		mappings = json.load(file)

else:
	with open(path_to_mtd / "mappings_MMOSA_MKTG_INiT.json") as file: 
		mappings = json.load(file)

MMOSA_MKTG_INiT = DAG(
	dag_id="MMOSA_MKTG_INiT", 
	default_args=default_args,
	description="MMOSA_MKTG_INiT", 
...
fmc_mtd = SnowflakeOperator(
	...
	sql=f"""CALL "MOSA_PROJECT_PROC"."SET_FMC_MTD_FL_INIT_MMKTG"('{{{{ dag_run.dag_id }}}}', '{{{{ dag_run.id }}}}', '{{{{ execution_date.strftime(\"%Y-%m-%d %H:%M:%S.%f\") }}}}');""", 
	autocommit=False, 
	dag=MMOSA_MKTG_INiT
)
...
if (path_to_mtd / "768_FL_mtd_MMOSA_MKTG_INiT_20251218_091550.json").exists():
	with open(path_to_mtd / "768_FL_mtd_MMOSA_MKTG_INiT_20251218_091550.json") as file: 
		analyze_data = json.load(file)
else:
	with open(path_to_mtd / "FL_mtd_MMOSA_MKTG_INiT.json") as file: 
		analyze_data = json.load(file)
