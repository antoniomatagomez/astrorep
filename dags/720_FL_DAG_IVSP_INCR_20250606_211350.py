...
Vaultspeed version: 5.7.3.18, generation date: 2025/06/06 21:13:50
...
default_args = {
	...
	"start_date":datetime.strptime("20-05-2025 06:00:00", "%d-%m-%Y %H:%M:%S")
}

path_to_mtd = Path(Variable.get("path_to_metadata"))

if (path_to_mtd / "720_mappings_IVSP_INCR_20250606_211350.json").exists():
	with open(path_to_mtd / "720_mappings_IVSP_INCR_20250606_211350.json") as file: 
		mappings = json.load(file)

else:
	with open(path_to_mtd / "mappings_IVSP_INCR.json") as file: 
		mappings = json.load(file)

IVSP_INCR = DAG(
	dag_id="IVSP_INCR", 
	default_args=default_args,
	description="IVSP_INCR", 
	schedule_interval='*/5 * * * *', 
	concurrency=3, 
	catchup=False, 
	max_active_runs=1,
...
fmc_mtd = SnowflakeOperator(
	...
	sql=f"""CALL "VSP_PROC"."SET_FMC_MTD_FL_INCR_ISLS"('{{{{ dag_run.dag_id }}}}', '{{{{ dag_run.id }}}}', '{{{{ data_interval_end.strftime(\"%Y-%m-%d %H:%M:%S.%f\") }}}}');""", 
	autocommit=False, 
	dag=IVSP_INCR
)
...
if (path_to_mtd / "720_FL_mtd_IVSP_INCR_20250606_211350.json").exists():
	with open(path_to_mtd / "720_FL_mtd_IVSP_INCR_20250606_211350.json") as file: 
		analyze_data = json.load(file)
else:
	with open(path_to_mtd / "FL_mtd_IVSP_INCR.json") as file: 
		analyze_data = json.load(file)
