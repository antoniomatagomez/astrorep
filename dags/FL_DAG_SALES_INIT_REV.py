...
Vaultspeed version: 6.0.0.4, generation date: 2026/03/11 10:19:42
...
default_args = {
	...
	"start_date":datetime.strptime("11-03-2026 06:00:00", "%d-%m-%Y %H:%M:%S")
}

path_to_mtd = Path(Variable.get("path_to_metadata"))

if (path_to_mtd / "799_mappings_SALES_INIT_REV_20260311_101942.json").exists():
	with open(path_to_mtd / "799_mappings_SALES_INIT_REV_20260311_101942.json") as file: 
		mappings = json.load(file)

else:
	with open(path_to_mtd / "mappings_SALES_INIT_REV.json") as file: 
		mappings = json.load(file)

SALES_INIT_REV = DAG(
	dag_id="SALES_INIT_REV", 
	default_args=default_args,
	description="SALES_INIT_REV", 
...
if (path_to_mtd / "799_FL_mtd_SALES_INIT_REV_20260311_101942.json").exists():
	with open(path_to_mtd / "799_FL_mtd_SALES_INIT_REV_20260311_101942.json") as file: 
		analyze_data = json.load(file)
else:
	with open(path_to_mtd / "FL_mtd_SALES_INIT_REV.json") as file: 
		analyze_data = json.load(file)
